...
CellInfo %>%   ggplot(aes(x=Cluster, fill=fct_rev(Patient))) +scale_fill_manual(values = SampleColors)+
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), legend.text = element_text(color="black",size=13,face = "bold"),legend.title = element_text(color="black",size=13,face = "bold"),
        axis.line = element_line(colour = "black"), axis.text.y = element_text(color="black",size=15),axis.text.x = element_text(color="black",angle = 45,hjust = 1,size=19),
        axis.text=element_text(size=15, face="bold"), axis.title=element_text(size=19,face="bold"), 
...
pdf(file =paste0(OutputDirectory,ObjName,Subset," pathway butterfly by Cluster fig4g.pdf"), height = 6, width =17,onefile = T)
