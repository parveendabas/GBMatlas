...
#markers = read.csv(paste0("~/Box/Yun lab projects/scRNAseq Project/Human Glioma/ALLSAMPLESJuly2021/Output/",Subset,"/",ObjName,Subset," cluster markers ","res",resolution,".csv"),row.names = 1)
...
mylegend<-get_legend(x3+theme_min()+ NoAxes()+guides(colour = guide_colourbar(title="Assignment",title.position = "top",title.theme = element_text(size=10))))
...
#Figure 3f
pdf(paste0(OutputDirectory,ObjName,Subset,"res",resolution,"violinplot fig 3f.pdf"),width = 6.5,height =4,family = "ArialMT")
VlnPlot(SeuratObj,features="PDCD1", group.by = "Patient", cols = SampleColors,log = T)+theme_min()+RotatedAxis()
dev.off()


saveRDS(SeuratObj,file = paste0(RobjDirectory,"TCellClusters-11-3-21 patients renamed.rds"))
